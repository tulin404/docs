...
export function Card({
    ...
    function getIcon() {
        switch(type) {
            case "project":
                return <Layers size={32} className="text-text" />
            case "api":
                return <Braces size={32} className="text-text" />
            case "module":
                return <Box size={32} className="text-text" />
            case "experiment":
                return <FlaskConical size={32} className="text-text" />
        };
    };
    ...
    return (
        <Link
            href=""
            className="group flex flex-col gap-6 p-5 border-2 border-border rounded-lg bg-linear-to-t from-background to-text-disabled/10 hover:border-border-hover transition-colors duration-200 hover:cursor-pointer"
        >
